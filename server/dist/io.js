import { pushNewOrder, getNextOrderForProcessing, getAllCurrentOrders, updateOrderStatus, markOrderAsReady, removeOrderCompletely, getPendingOrders, getProcessingOrders, getReadyOrders } from "./socketFunctions/OrderHandler.service.js";
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("ORDER_CREATE", (data) => {
            try {
                ...
                socket.emit("ORDER_CREATE_FAILED", { message: error.message });
            }
        });
        socket.on("ORDER_TAKE_NEXT", () => {
            try {
                ...
                socket.emit("ORDER_TAKE_NEXT_FAILED", { message: error.message });
            }
        });
        socket.on("ORDER_UPDATE_STATUS", (data) => {
            try {
                ...
                socket.emit("ORDER_UPDATE_STATUS_FAILED", { message: error.message });
            }
        });
        socket.on("ORDER_MARK_READY", (orderId) => {
            try {
                ...
                socket.emit("ORDER_MARK_READY_FAILED", { message: error.message });
            }
        });
        socket.on("ORDER_REMOVE_COMPLETED", (orderId) => {
            try {
                ...
                socket.emit("ORDER_REMOVE_COMPLETED_FAILED", { message: error.message });
            }
        });
